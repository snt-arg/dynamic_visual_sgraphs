...
namespace ORB_SLAM3
{
    ...
    {
        std::cout << "[SysParams] Loading system parameters from " << strConfigFile << std::endl;
        try
        {
        ...
        {
            std::cout << "[SysParams] Error loading configuration file " << e.what() << std::endl;
        ...
        try
        {
            ...
            room_seg.cluster_centroid_wall_centroid_distance_thresh = mConfig["room_seg"]["skeleton_based"]["cluster_centroid_wall_centroid_distance_thresh"].as<float>();
        }
